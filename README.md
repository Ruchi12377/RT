...
このシステムは、ユーザーの指定した範囲もしくは、全範囲(つまり全消し。今後実装予定)をレールや枠組みに沿って本体が移動し自動で白板に描かれたものを消すものである。

そして、制御部分にはRTミドルウェアを用いており、cameraコンポーネントを用いて取得した白板の動画(画像)をselectコンポーネントで消す範囲を選択し、moveコンポーネントで実際に動かす。というものである。
